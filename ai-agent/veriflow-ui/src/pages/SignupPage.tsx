...
import { Eye, EyeOff, Sparkles, ArrowRight, Loader2, CheckCircle2 } from 'lucide-react'
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidReqs = validatePassword().filter((r) => !r.valid)
  ...
  return (
    <div className="min-h-screen bg-dark-950 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-neon-green/10 via-dark-950 to-dark-950" />
        <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-neon-green/20 rounded-full blur-[120px] animate-pulse-slow" />
        <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-neon-cyan/20 rounded-full blur-[120px] animate-pulse-slow delay-1000" />
      </div>
      ...
      >
        ...
        <div className="glass-card p-8 rounded-2xl neon-border">
          <div className="text-center mb-8">
            ...
            <p className="text-dark-400">Start your AI testing journey today</p>
          </div>
                  ...
                  {validatePassword().map((req, i) => (
                    <div
                      ...
                      className={`flex items-center gap-2 text-xs ${
                        req.valid ? 'text-neon-green' : 'text-dark-400'
                      }`}
                    >
                      <CheckCircle2 className={`w-3 h-3 ${req.valid ? 'opacity-100' : 'opacity-50'}`} />
                      {req.text}
                    </div>
                  ))}
                </motion.div>
